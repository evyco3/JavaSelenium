 ...
 *
 * <p>The tests navigate from the homepage clicking account dropdown->clicking the login link button -> landing in login page,
...
public class LoginTest extends BaseTest {
    ...
    @Step("Perform login and get response message for email: {email}, password: {password}, operation: {operation}")
    private String performLoginAndGetResponseMessage(String email, String password, String operation) {
        try {
            ...
            LoggerUtils.log(getClass(), LogType.ERROR, "Error During login test process " + e);
            throw new RuntimeException("Error During login test process ", e);
        }
    }
}
